...
def test_win11(nr_samples=200,file_name='bm869_1.pkl'):
  data = np.empty(nr_samples)
  hid.enumerate()
  rr=hid.device(0x820,1)
  rr.open(0x820,1)
...
def plot(f_name='bm869_4v8_Vdc.pkl'):
  # plots samples in time domain
  f = t.fast_file()
  data_set = f.load (f_name)
  data_set = np.divide(data_set,10)  
  Fs = 5
  len = data_set.size
  t1 = np.linspace(start=0, stop=len/Fs, num=len,) # time in seconds  
  print ('Sigma=',np.std(data_set),'Begin=',data_set[1],'End=',data_set[len-1])
  plt.plot (t1,data_set)
  plt.title ('Sample[V] vs time[s]')
  plt.grid ()
  plt.show()
